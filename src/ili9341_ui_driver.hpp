...
class ILI9341UiDriver : public _UiDriver
{
  public:
                              ...
                              uint8_t color)
    {
        uint8_t alpha = 255;
        if (r == 0) {
            return DrawRect(x, y, w, h, color, alpha);
        }
        if (r * 2 > w) r = w / 2;
        if (r * 2 > h) r = h / 2;
        ...
        DrawCircleQuarter(x + r, y + r, r, color, Quadrant::TopLeft, alpha);
        DrawCircleQuarter(x + w - r - 1, y + r, r, color, Quadrant::TopRight, alpha);
        DrawCircleQuarter(x + r, y + h - r - 1, r, color, Quadrant::BottomLeft, alpha);
        DrawCircleQuarter(x + w - r - 1, y + h - r - 1, r, color, Quadrant::BottomRight, alpha);
    }
    // Fills a rounded rectangle
    void FillRoundedRectangle(uint16_t x,
                              uint16_t y,
                              uint16_t w,
                              uint16_t h,
                              uint16_t r,
                              uint8_t color)
    {
        uint8_t alpha = 255;
        if (r == 0) {
            return FillRect(Rectangle(x, y, w, h), color, alpha);
        }
        if (r * 2 > w) r = w / 2;
        if (r * 2 > h) r = h / 2;
  ...
  private:
    ...
    void DrawCircleQuarter(int32_t cx,
                           int32_t cy,
                           int32_t radius,
                           uint8_t color,
                           Quadrant q,
                           uint8_t alpha=255)
    {
        ...
        for (int deg = 0; deg < 90; ++ deg) {
            ...
            int dx = (int)roundf(cosf(rad) * radius);
            int dy = (int)roundf(sinf(rad) * radius);
            int px = 0, py = 0;
            switch (q) {
